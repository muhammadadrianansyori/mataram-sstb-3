...
class BoundaryManager:
    ...
    def spatial_filter(self, detections: List[Dict], kelurahan_names: List[str], lingkungan_names: List[str] = None, rt_names: List[str] = None) -> List[Dict]:
        """
        Filter detections with granular control (Kelurahan -> Lingkungan -> RT)
        """
        if self._gdf is None:
            return detections
            
        if not kelurahan_names:
            return detections
